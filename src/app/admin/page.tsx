...
export default function AdminPage() {
  ...
  const [showCrearBonificacion, setShowCrearBonificacion] = useState(false);
  const [bonificacionUsuarioSearch, setBonificacionUsuarioSearch] = useState('');
  const [bonificacionUsuarioSeleccionado, setBonificacionUsuarioSeleccionado] = useState<Usuario | null>(null);
  const [bonificacionParaTodos, setBonificacionParaTodos] = useState(false);
  const [nuevaBonificacion, setNuevaBonificacion] = useState({
    tipo: 'dias' as 'dias' | 'anuncios' | 'promocion_gratis' | 'descuento' | 'regalo',
    cantidad: 7,
    motivo: '',
    planTipo: 'Destacado',
  ...
  // Estados para créditos/puntos
  const [showDarCreditos, setShowDarCreditos] = useState(false);
  const [creditosUsuarioSearch, setCreditosUsuarioSearch] = useState('');
  const [creditosUsuarioSeleccionado, setCreditosUsuarioSeleccionado] = useState<Usuario | null>(null);
  const [cantidadCreditos, setCantidadCreditos] = useState(50);
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className={`transition-all ${sidebarOpen ? 'ml-64' : 'ml-0'}`}>
        ...
        <div className="p-6">
          ...
          {showDarCreditos && (
            <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
              <div className="bg-white rounded-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto">
                <div className="p-6 border-b border-gray-100 sticky top-0 bg-white z-10">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h3 className="text-xl font-bold text-gray-900">Dar Créditos</h3>
                        <p className="text-sm text-gray-500">Añade créditos a uno o todos los usuarios</p>
                      </div>
                    </div>
                    <button
                      onClick={() => {
                ...
                <div className="p-6 space-y-6">
                  {/* Opción: Todos los usuarios o usuario específico */}
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">
                      Destinatario
                    </label>
                    <div className="flex gap-3">
                      ...
                      >
                        <User size={24} className={creditosParaTodos ? 'text-gray-400' : 'text-amber-600'} />
                        <div className="text-left">
                          <p className={`font-semibold ${creditosParaTodos ? 'text-gray-600' : 'text-amber-900'}`}>Usuario Específico</p>
                          <p className="text-xs text-gray-500">Dar puntos a un usuario</p>
                        </div>
                      </button>
                      <button
                        onClick={() => {
                      ...
                      >
                        <Users size={24} className={creditosParaTodos ? 'text-amber-600' : 'text-gray-400'} />
                        <div className="text-left">
                          <p className={`font-semibold ${creditosParaTodos ? 'text-amber-900' : 'text-gray-600'}`}>Todos los Usuarios</p>
                          <p className="text-xs text-gray-500">{usuarios.length} usuarios activos</p>
                        </div>
                      </button>
                    </div>
                  </div>

                  {/* Buscar usuario - solo si no es para todos */}
                  {!creditosParaTodos && (
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">
                      Seleccionar Usuario
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        value={creditosUsuarioSearch}
                        onChange={(e) => setCreditosUsuarioSearch(e.target.value)}
                    ...
                    {creditosUsuarioSearch.length > 2 && !creditosUsuarioSeleccionado && (
                      <div className="mt-2 bg-white border border-gray-200 rounded-xl max-h-40 overflow-y-auto shadow-lg">
                        {usuarios
                          .filter(u => 
                            u.nombre?.toLowerCase().includes(creditosUsuarioSearch.toLowerCase()) ||
                            u.email?.toLowerCase().includes(creditosUsuarioSearch.toLowerCase())
                          )
                          .slice(0, 5)
                          .map(u => (
                            <button
                              key={u.id}
                              onClick={() => {
                                setCreditosUsuarioSeleccionado(u);
                                setCreditosUsuarioSearch(u.nombre || u.email || '');
                              }}
                              className="w-full px-4 py-3 text-left hover:bg-amber-50 border-b border-gray-100 last:border-0"
                            >
                        ...
                        {usuarios.filter(u => 
                          ...
                          <p className="px-4 py-3 text-sm text-gray-500">No se encontraron usuarios</p>
                        )}
                      </div>
                    )}
                    
                    {/* Usuario seleccionado */}
                    {creditosUsuarioSeleccionado && (
                      <div className="mt-3 p-3 bg-amber-50 border border-amber-200 rounded-xl flex items-center justify-between">
                        <div>
                          <p className="font-semibold text-amber-900">{creditosUsuarioSeleccionado.nombre}</p>
                          <p className="text-sm text-amber-700">{creditosUsuarioSeleccionado.email}</p>
                        </div>
                        <button
                          onClick={() => {
                            setCreditosUsuarioSeleccionado(null);
                  ...
                  <div>
                    ...
                    <div className="flex gap-2">
                      {[10, 25, 50, 100, 200].map(cantidad => (
                        <button
                          key={cantidad}
                          onClick={() => setCantidadCreditos(cantidad)}
                          className={`px-4 py-2 rounded-lg font-medium transition-all ${
                    ...
                    <input
                      ...
                      onChange={(e) => setCantidadCreditos(parseInt(e.target.value) || 0)}
                      min="1"
                      className="mt-2 w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-amber-500 focus:border-transparent"
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      rows={2}
                      placeholder="Ej: Compensación, regalo especial, etc."
                    />
                  </div>
                  ...
                  <div className="bg-amber-50 rounded-xl p-4">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className="text-2xl font-bold text-amber-600 flex items-center gap-1">
                        <span>🪙</span> {cantidadCreditos} {creditosParaTodos && <span className="text-sm font-normal">x {usuarios.length} usuarios</span>}
                      </span>
                    </div>
                    <div className="text-xs text-amber-600 space-y-1">
                      <p>💵 Valor: <span className="font-bold">{(cantidadCreditos * configCreditos.valorPuntosEnEuros).toFixed(2)}€</span> {creditosParaTodos && <span className="text-amber-500">(Total: {(cantidadCreditos * configCreditos.valorPuntosEnEuros * usuarios.length).toFixed(2)}€)</span>}</p>
                      <p>📢 = <span className="font-bold">{Math.floor((cantidadCreditos * configCreditos.valorPuntosEnEuros) / configCreditos.precioAnuncioEnEuros)}</span> anuncios extra</p>
                ...
                <div className="p-6 border-t border-gray-100 flex gap-3">
                  ...
                  <button
                    onClick={async () => {
                      ...
                      if (!creditosParaTodos && !creditosUsuarioSeleccionado?.id) {
                        toastWarning('Usuario requerido', 'Por favor selecciona un usuario');
                        return;
                      }
          ...
          {/* Modal Crear Bonificación */}
          {showCrearBonificacion && (
            <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
              <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
                <div className="p-6 border-b border-gray-100 sticky top-0 bg-white z-10">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h3 className="text-xl font-bold text-gray-900">Nueva Bonificación</h3>
                        <p className="text-sm text-gray-500">Crea una bonificación para un usuario</p>
                      </div>
                    </div>
                    <button
                      onClick={() => {
                        setShowCrearBonificacion(false);
                        setBonificacionUsuarioSeleccionado(null);
                        setBonificacionUsuarioSearch('');
                      }}
                      className="p-2 hover:bg-gray-100 rounded-lg transition-colors"
                    >
                ...
                <div className="p-6 space-y-6">
                  ...
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-2">
                      1. Destinatario de la Bonificación
                    </label>
                    ...
                    <div className="flex gap-3 mb-4">
                      <button
                        onClick={() => {
                          setBonificacionParaTodos(false);
                          setBonificacionUsuarioSeleccionado(null);
                        }}
                        className={`flex-1 p-4 rounded-xl border-2 transition-all flex items-center gap-3 ${
                          !bonificacionParaTodos
                            ? 'border-emerald-500 bg-emerald-50'
                            : 'border-gray-200 hover:border-gray-300'
                        }`}
                      >
                        <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${
                          !bonificacionParaTodos ? 'bg-emerald-500' : 'bg-gray-200'
                        }`}>
                          <Users size={20} className={!bonificacionParaTodos ? 'text-white' : 'text-gray-500'} />
                        </div>
                        <div className="text-left">
                          <p className={`font-semibold ${!bonificacionParaTodos ? 'text-emerald-700' : 'text-gray-700'}`}>
                            Usuario Específico
                          </p>
                          <p className="text-xs text-gray-500">Bonificación para un usuario</p>
                        </div>
                      </button>
                      <button
                        onClick={() => {
                          setBonificacionParaTodos(true);
                          setBonificacionUsuarioSeleccionado(null);
                          setBonificacionUsuarioSearch('');
                        }}
                        className={`flex-1 p-4 rounded-xl border-2 transition-all flex items-center gap-3 ${
                      ...
                      >
                        <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${
                          bonificacionParaTodos ? 'bg-purple-500' : 'bg-gray-200'
                        }`}>
                          <Sparkles size={20} className={bonificacionParaTodos ? 'text-white' : 'text-gray-500'} />
                        </div>
                        <div className="text-left">
                          <p className={`font-semibold ${bonificacionParaTodos ? 'text-purple-700' : 'text-gray-700'}`}>
                            Todos los Usuarios
                          </p>
                          <p className="text-xs text-gray-500">Bonificación masiva ({usuarios.length} usuarios)</p>
                        </div>
                      </button>
                    </div>

                    {/* Selección de usuario específico */}
                    {!bonificacionParaTodos && !bonificacionUsuarioSeleccionado && (
                      <div>
                        <div className="relative">
                          <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" size={18} />
                          <input
                            type="text"
                            placeholder="Buscar por nombre o email..."
                            value={bonificacionUsuarioSearch}
                            onChange={(e) => setBonificacionUsuarioSearch(e.target.value)}
                        ...
                        {bonificacionUsuarioSearch.length >= 2 && (
                          <div className="mt-2 border border-gray-200 rounded-xl max-h-[200px] overflow-y-auto">
                            {usuarios
                              .filter(u => 
                                u.nombre?.toLowerCase().includes(bonificacionUsuarioSearch.toLowerCase()) ||
                                u.email?.toLowerCase().includes(bonificacionUsuarioSearch.toLowerCase())
                              )
                              .slice(0, 10)
                              .map(u => (
                                <button
                                  key={u.id}
                                  onClick={() => {
                                    setBonificacionUsuarioSeleccionado(u);
                                    setBonificacionUsuarioSearch('');
                                  }}
                            ...
                            {usuarios.filter(u => 
                              ...
                              <p className="p-4 text-center text-gray-500 text-sm">No se encontraron usuarios</p>
                            )}
                          </div>
                        )}
                      </div>
                        ...
                        <div className="flex items-center gap-3">
                          <div className="w-12 h-12 bg-emerald-500 rounded-full flex items-center justify-center">
                            <span className="font-bold text-white text-lg">
                              {bonificacionUsuarioSeleccionado.nombre?.charAt(0).toUpperCase() || '?'}
                            </span>
                          </div>
                          <div>
                            <p className="font-semibold text-gray-900">{bonificacionUsuarioSeleccionado.nombre}</p>
                            <p className="text-sm text-gray-600">{bonificacionUsuarioSeleccionado.email}</p>
                          </div>
                        </div>
                        <button
                          onClick={() => setBonificacionUsuarioSeleccionado(null)}
                    ...
                    {/* Todos los usuarios seleccionado */}
                    {bonificacionParaTodos && (
                      <div className="p-4 bg-gradient-to-r from-purple-50 to-pink-50 rounded-xl border border-purple-200">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex-1">
                            <p className="font-semibold text-gray-900">Todos los Usuarios</p>
                            <p className="text-sm text-gray-600">Se aplicará a <span className="font-bold text-purple-600">{usuarios.length}</span> usuarios activos</p>
                          </div>
                        ...
                        <div className="mt-3 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                          <p className="text-xs text-yellow-800 flex items-start gap-2">
                            ...
                            <span>Esta bonificación se aplicará a <strong>todos</strong> los usuarios registrados. Esta acción creará {usuarios.length} bonificaciones individuales.</span>
                          </p>
                        </div>
                      </div>
                    )}
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                      <button
                        onClick={() => setNuevaBonificacion(prev => ({ ...prev, tipo: 'dias' }))}
                        className={`p-4 rounded-xl border-2 transition-all text-left ${
                          nuevaBonificacion.tipo === 'dias'
                            ? 'border-blue-500 bg-blue-50'
                            : 'border-gray-200 hover:border-gray-300'
                        }`}
                      >
                        <Calendar size={24} className={nuevaBonificacion.tipo === 'dias' ? 'text-blue-500' : 'text-gray-400'} />
                        <p className={`font-medium mt-2 ${nuevaBonificacion.tipo === 'dias' ? 'text-blue-700' : 'text-gray-700'}`}>
                          Días Extra
                        </p>
                      </button>
                      <button
                        onClick={() => setNuevaBonificacion(prev => ({ ...prev, tipo: 'anuncios' }))}
                        className={`p-4 rounded-xl border-2 transition-all text-left ${
                          nuevaBonificacion.tipo === 'anuncios'
                            ? 'border-purple-500 bg-purple-50'
                            : 'border-gray-200 hover:border-gray-300'
                        }`}
                      >
                        <FileText size={24} className={nuevaBonificacion.tipo === 'anuncios' ? 'text-purple-500' : 'text-gray-400'} />
                        <p className={`font-medium mt-2 ${nuevaBonificacion.tipo === 'anuncios' ? 'text-purple-700' : 'text-gray-700'}`}>
                          Anuncios Extra
                        </p>
                      </button>
                      <button
                        onClick={() => setNuevaBonificacion(prev => ({ ...prev, tipo: 'promocion_gratis' }))}
                        className={`p-4 rounded-xl border-2 transition-all text-left ${
                          nuevaBonificacion.tipo === 'promocion_gratis'
                            ? 'border-pink-500 bg-pink-50'
                            : 'border-gray-200 hover:border-gray-300'
                        }`}
                      >
                        <Sparkles size={24} className={nuevaBonificacion.tipo === 'promocion_gratis' ? 'text-pink-500' : 'text-gray-400'} />
                        <p className={`font-medium mt-2 ${nuevaBonificacion.tipo === 'promocion_gratis' ? 'text-pink-700' : 'text-gray-700'}`}>
                          Promo Gratis
                        </p>
                      </button>
                      <button
                        onClick={() => setNuevaBonificacion(prev => ({ ...prev, tipo: 'descuento' }))}
                        className={`p-4 rounded-xl border-2 transition-all text-left ${
                          nuevaBonificacion.tipo === 'descuento'
                            ? 'border-green-500 bg-green-50'
                            : 'border-gray-200 hover:border-gray-300'
                        }`}
                      >
                        <DollarSign size={24} className={nuevaBonificacion.tipo === 'descuento' ? 'text-green-500' : 'text-gray-400'} />
                        <p className={`font-medium mt-2 ${nuevaBonificacion.tipo === 'descuento' ? 'text-green-700' : 'text-gray-700'}`}>
                          Descuento
                        </p>
                      </button>
                      <button
                        onClick={() => setNuevaBonificacion(prev => ({ ...prev, tipo: 'regalo' }))}
                        className={`p-4 rounded-xl border-2 transition-all text-left ${
                          nuevaBonificacion.tipo === 'regalo'
                            ? 'border-indigo-500 bg-indigo-50'
                            : 'border-gray-200 hover:border-gray-300'
                        }`}
                      >
                        <Gift size={24} className={nuevaBonificacion.tipo === 'regalo' ? 'text-indigo-500' : 'text-gray-400'} />
                        <p className={`font-medium mt-2 ${nuevaBonificacion.tipo === 'regalo' ? 'text-indigo-700' : 'text-gray-700'}`}>
                          Regalo Especial
                        </p>
                      </button>
                    </div>
                  ...
                  <div>
                    ...
                    <div className="space-y-4">
                      {(nuevaBonificacion.tipo === 'dias' || nuevaBonificacion.tipo === 'anuncios') && (
                        <div>
                          <label className="text-sm text-gray-600 block mb-1">
                            Cantidad de {nuevaBonificacion.tipo === 'dias' ? 'días' : 'anuncios'}
                          </label>
                          <input
                            type="number"
                            min="1"
                            value={nuevaBonificacion.cantidad}
                            onChange={(e) => setNuevaBonificacion(prev => ({ ...prev, cantidad: parseInt(e.target.value) || 1 }))}
                      ...
                      {nuevaBonificacion.tipo === 'promocion_gratis' && (
                        <div>
                          <label className="text-sm text-gray-600 block mb-1">Tipo de promoción a regalar</label>
                          <select
                            value={nuevaBonificacion.planTipo}
                            onChange={(e) => setNuevaBonificacion(prev => ({ ...prev, planTipo: e.target.value }))}
                            className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500"
                          >
                            <option value="Destacado">Destacado (7 días)</option>
                            <option value="Premium">Premium (15 días)</option>
                            <option value="VIP">VIP (30 días)</option>
                          </select>
                          <p className="text-xs text-gray-500 mt-2">El usuario podrá usar esta promoción gratuita en cualquiera de sus anuncios.</p>
                        </div>
                      )}
                      ...
                      {nuevaBonificacion.tipo === 'descuento' && (
                        <>
                          <div>
                            <label className="text-sm text-gray-600 block mb-1">Porcentaje de descuento</label>
                            <div className="relative">
                              <input
                                type="number"
                                min="1"
                                max="100"
                                value={nuevaBonificacion.porcentajeDescuento}
                                onChange={(e) => setNuevaBonificacion(prev => ({ ...prev, porcentajeDescuento: parseInt(e.target.value) || 10 }))}
                                className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500"
                              />
                          ...
                          <div>
                            <label className="text-sm text-gray-600 block mb-1">Código promocional (opcional)</label>
                            <input
                              type="text"
                              placeholder="Ej: WELCOME20"
                              value={nuevaBonificacion.codigoPromo}
                              onChange={(e) => setNuevaBonificacion(prev => ({ ...prev, codigoPromo: e.target.value.toUpperCase() }))}
                              className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 font-mono"
                            />
                          </div>
                        </>
                      )}
                      ...
                      <div>
                        <label className="text-sm text-gray-600 block mb-1">Días de validez</label>
                        <input
                          type="number"
                          min="1"
                          value={nuevaBonificacion.diasValidez}
                          onChange={(e) => setNuevaBonificacion(prev => ({ ...prev, diasValidez: parseInt(e.target.value) || 30 }))}
                          className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500"
                        />
                      </div>
                    </div>
                  ...
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-2">
                      4. Motivo de la Bonificación *
                    </label>
                    
                    {/* Motivos predefinidos */}
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-2 mb-3">
                      {[
                        { label: '🎉 Bienvenida', value: 'Bonificación de bienvenida a la plataforma' },
                        { label: '🙏 Fidelidad', value: 'Regalo de fidelidad por ser usuario activo' },
                        { label: '🔧 Compensación', value: 'Compensación por incidencia técnica' },
                        { label: '🎄 Navidad', value: 'Bonificación especial de Navidad' },
                        { label: '🎂 Aniversario', value: 'Bonificación por aniversario en la plataforma' },
                        { label: '⭐ Usuario VIP', value: 'Bonificación especial para usuario VIP' },
                        { label: '🏆 Ganador sorteo', value: 'Premio por ganar sorteo promocional' },
                        { label: '💬 Feedback', value: 'Agradecimiento por proporcionar feedback valioso' },
                        { label: '🐛 Reporte bug', value: 'Recompensa por reportar un error en la plataforma' },
                        { label: '📢 Referido', value: 'Bonificación por referir nuevos usuarios' },
                        { label: '🚀 Early adopter', value: 'Bonificación por ser usuario desde el inicio' },
                        { label: '✏️ Personalizado', value: '' },
                      ].map((motivo) => (
                        <button
                          key={motivo.label}
                          type="button"
                          onClick={() => {
                            if (motivo.value) {
                              setNuevaBonificacion(prev => ({ ...prev, motivo: motivo.value }));
                            } else {
                              setNuevaBonificacion(prev => ({ ...prev, motivo: '' }));
                            }
                          }}
                          className={`p-2 text-sm rounded-lg border-2 transition-all text-left ${
                            nuevaBonificacion.motivo === motivo.value || (motivo.value === '' && !['Bonificación de bienvenida a la plataforma', 'Regalo de fidelidad por ser usuario activo', 'Compensación por incidencia técnica', 'Bonificación especial de Navidad', 'Bonificación por aniversario en la plataforma', 'Bonificación especial para usuario VIP', 'Premio por ganar sorteo promocional', 'Agradecimiento por proporcionar feedback valioso', 'Recompensa por reportar un error en la plataforma', 'Bonificación por referir nuevos usuarios', 'Bonificación por ser usuario desde el inicio'].includes(nuevaBonificacion.motivo) && nuevaBonificacion.motivo !== '')
                    ...
                    {/* Campo de texto para motivo personalizado o editar */}
                    <textarea
                      value={nuevaBonificacion.motivo}
                      onChange={(e) => setNuevaBonificacion(prev => ({ ...prev, motivo: e.target.value }))}
                      placeholder="Escribe un motivo personalizado o selecciona uno de arriba..."
                      rows={2}
                      className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <textarea
                      value={nuevaBonificacion.notas}
                      onChange={(e) => setNuevaBonificacion(prev => ({ ...prev, notas: e.target.value }))}
                ...
                <div className="p-6 border-t border-gray-100 sticky bottom-0 bg-white flex gap-3">
                  ...
                  <button
                    onClick={async () => {
                      // Validaciones
                      if (!bonificacionParaTodos && !bonificacionUsuarioSeleccionado) {
                        toastWarning('Usuario requerido', 'Por favor selecciona un usuario o elige "Todos los usuarios"');
                        return;
                      }
                      if (!nuevaBonificacion.motivo.trim()) {
                        toastWarning('Motivo requerido', 'Por favor indica el motivo de la bonificación');
                        return;
                      }

                      // Confirmación para bonificación masiva
                      if (bonificacionParaTodos) {
                        const confirmar = confirm(
                          `⚠️ BONIFICACIÓN MASIVA\n\n` +
                          `Estás a punto de crear una bonificación de tipo "${nuevaBonificacion.tipo}" para TODOS los ${usuarios.length} usuarios.\n\n` +
                          `Cantidad: ${nuevaBonificacion.tipo === 'descuento' ? nuevaBonificacion.porcentajeDescuento + '%' : '+' + nuevaBonificacion.cantidad}\n` +
                          `Motivo: ${nuevaBonificacion.motivo}\n\n` +
                          `¿Deseas continuar?`
                        );
                        if (!confirmar) return;
                      }
                      ...
                      try {
                        if (bonificacionParaTodos) {
                          // Crear bonificación masiva en Firebase para cada usuario
                          const bonificacionBase = {
                            tipo: nuevaBonificacion.tipo,
                            cantidad: nuevaBonificacion.cantidad,
                            motivo: nuevaBonificacion.motivo,
                            fechaExpiracion: new Date(Date.now() + nuevaBonificacion.diasValidez * 24 * 60 * 60 * 1000),
                            estado: 'activa' as const,
                            creadoPor: user?.uid || '',
                            planTipo: nuevaBonificacion.planTipo,
                            porcentajeDescuento: nuevaBonificacion.porcentajeDescuento,
                            codigoPromo: nuevaBonificacion.codigoPromo || undefined,
                            notas: nuevaBonificacion.notas || undefined,
                          };

                          const usuariosParaBonificacion = usuarios.map(u => ({
                            id: u.id!,
                            nombre: u.nombre,
                            email: u.email
                          }));

                          const resultado = await crearBonificacionMasiva(bonificacionBase, usuariosParaBonificacion);
                          
                          // Log de la acción masiva
                          await logAdminAction(
                            user?.uid || '',
                            user?.email || '',
                            'crear_bonificacion_masiva',
                            'sistema',
                            'bonif_masiva',
                            `Bonificación masiva de tipo ${nuevaBonificacion.tipo} creada para ${resultado.exitosos} usuarios (${resultado.fallidos} fallidos)`
                          );
                          
                          // Recargar bonificaciones desde Firebase
                          const bonificacionesActualizadas = await getAllBonificaciones();
                          setBonificaciones(bonificacionesActualizadas);
                          
                          toastSuccess('¡Bonificación masiva creada!', `Enviada exitosamente a ${resultado.exitosos} usuarios`);
                        } else {
                          // Crear bonificación individual en Firebase
                          const nuevaBonif = {
                            usuarioId: bonificacionUsuarioSeleccionado!.id!,
                            usuarioNombre: bonificacionUsuarioSeleccionado!.nombre || '',
                            usuarioEmail: bonificacionUsuarioSeleccionado!.email || '',
                            tipo: nuevaBonificacion.tipo,
                            cantidad: nuevaBonificacion.cantidad,
                            motivo: nuevaBonificacion.motivo,
                            fechaExpiracion: new Date(Date.now() + nuevaBonificacion.diasValidez * 24 * 60 * 60 * 1000),
                          ...
                          const bonificacionId = await crearBonificacion(nuevaBonif);
                          ...
                          await logAdminAction(
                            ...
                            'sistema',
                            bonificacionId,
                            `Bonificación de tipo ${nuevaBonificacion.tipo} creada para ${bonificacionUsuarioSeleccionado!.nombre}`
                          );
                          ...
                          toastSuccess('¡Bonificación creada!', `Enviada a ${bonificacionUsuarioSeleccionado!.nombre}`);
                        }
                      } catch (error) {
                        console.error('Error al crear bonificación:', error);
                        toastError('Error', 'No se pudo crear la bonificación. Revisa la consola.');
                        return;
                      }
                    ...
                    disabled={(!bonificacionParaTodos && !bonificacionUsuarioSeleccionado) || !nuevaBonificacion.motivo.trim()}
                    className={`flex-1 px-6 py-3 text-white rounded-xl font-semibold transition-all disabled:opacity-50 disabled:cursor-not-allowed ${
                      bonificacionParaTodos 
                  ...
                  >
                    {bonificacionParaTodos ? (
                      <span className="flex items-center justify-center gap-2">
                        <Sparkles size={18} />
                        Crear para {usuarios.length} usuarios
                      </span>
                    ) : (
                      'Crear Bonificación'
                    )}
